...
class DeepWoundsEnchant extends ReactiveEnchantment
{
    ...
    {
        return ["interval" => 20, "durationMultiplier" => 20, "base" => 1, "multiplier" => 0.022];
    }
    ...
    public function react(Player $player, Item $item, Inventory $inventory, int $slot, Event $event, int $level, int $stack): void
    {
        if ($event instanceof EntityDamageByEntityEvent) {
			...
			if($entity instanceof Living) {
				...
				$player->sendMessage("§b*§d*§b* §6Wounding The Enemy! §8(§7Constant Bleed Damage§8) §b*§d*§b*");
				$event->getEntity()->sendMessage("§c*§6*§c* §cThe Enemy Has Wounded You! §8(§7Constant Bleed Damage§8) §c*§6*§c*");
			}
		}
	}
}