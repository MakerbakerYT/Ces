...
class InsomniaEnchant extends ReactiveEnchantment
{
    /** @var int */
    public $maxLevel = 7;
    /** @var int */
    ...
    /** @var array */
    private $effectIds = [Effect::NAUSEA, Effect::MINING_FATIGUE];
    /** @var array */
     ...
     */
    public function __construct(PiggyCustomEnchants $plugin, int $id, string $name, int $rarity = self::RARITY_SIMPLE, array $effectIds = [Effect::NAUSEA, Effect::MINING_FATIGUE], array $durationMultiplier = [60], array $aphlifierMultiplier = [1], array $baseDuration = [0], array $baseAphlifier = [0])
    {
        $this->name = $name;
        $this->rarity = $rarity;
    ...
    public function react(Player $player, Item $item, Inventory $inventory, int $slot, Event $event, int $level, int $stack): void
    {
        if ($event instanceof EntityDamageByEntityEvent) {
            ...
            if ($entity instanceof Living) {
                ...
                $player->sendMessage("§b*§d*§b* §7Inflicted Insomnia! §8(§7Confusion Buffed§8) §b*§d*§b*");
		        $event->getEntity()->sendMessage("§c*§6*§c* §cThe Enemy Has Inflicted Insomnia on You! §8(§7Confusion Buffed§8) §c*§6*§c*");
			}
		}
	}
}