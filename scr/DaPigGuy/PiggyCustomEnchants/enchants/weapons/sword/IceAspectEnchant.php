...
class IceAspectEnchant extends ReactiveEnchantment
{
    /** @var array */
     ...
     */
    public function __construct(PiggyCustomEnchants $plugin, int $id, string $name, int $rarity = self::RARITY_ULTIMATE, array $effectIds = [Effect::SLOWNESS], array $durationMultiplier = [60], array $amplifierMultiplier = [1], array $baseDuration = [0], array $baseAmplifier = [0])
    {
        $this->name = $name;
        $this->rarity = $rarity;
    ...
    public function react(Player $player, Item $item, Inventory $inventory, int $slot, Event $event, int $level, int $stack): void
    {
        if ($event instanceof EntityDamageByEntityEvent) {
            ...
            if ($entity instanceof Living) {
				...
				$player->sendMessage("§b*§d*§b* §eIced The Enemy! §8(§7Frozen In Place§8) §b*§d*§b*");
	    		$event->getEntity()->sendMessage("§c*§6*§c* §cThe Enemy Has Froze You! §8(§7Frozen In Place§8) §c*§6*§c*");
			}
		}
	}
}