...
class DeepBleedEnchant extends ReactiveEnchantment
{
    /** @var string */
    public $name = "Deep Bleed";
    /** @var int */
    public $maxLevel = 6;
    /** @var int */
    ...
    {
        return ["interval" => 20, "durationMultiplier" => 20, "base" => 1, "multiplier" => 0.99];
    }
			...
			if($entity instanceof Living) {
				...
				if (!isset(self::$tasks[$entity->getId()])) {
					...
					self::$tasks[$entity->getId()] = new ClosureTask(function () use ($entity, $endTime): void {
                        ...
                        $entity->getLevel()->addParticle(new ExplodeParticle($entity->add(0, 1), Particle::get(Particle::TYPE_EXPLODE)));
					});
					$this->plugin->getScheduler()->scheduleRepeatingTask(self::$tasks[$entity->getId()], $this->extraData["interval"]);
				}
				$player->sendMessage("§b*§d*§b* §dDeep Bleeding The Enemy! §8(§7Heroic Bleed§8) §b*§d*§b*");
				$event->getEntity()->sendMessage("§c*§6*§c* §cThe Enemy Has Made You Deep Bleed! §8(§7Heroic Bleed§8) §c*§6*§c*");
			}
		}
	}
}